  ...
  const handleSelect = async (option: string) => {
    ...
    try {
      ...
      if (data.type === "question") {
        setMessages((prev) => [
          ...prev,
          { role: "ai", content: data.text, options: data.options },
        ]);
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {msg.options.map((opt) => (
                      <button
                        ...
                        className="px-4 py-2 rounded-full border text-xs md:text-sm font-medium transition-all duration-200 hover:scale-[1.02]"
                        style={{
                          borderColor: "#e6c419",
                          color: "#e6c419",
                        }}
